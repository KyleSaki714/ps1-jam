...
func set_mouse_motion(mm : InputEventMouseMotion, ms : float):
	_mouseMotion = mm
	_mouseSens = ms
...
func _physics_process(delta: float) -> void:
	...
	# --- Mouse Motion ---
	if _mouseMotion.relative.length() > 0.6:
		print(_mouseMotion.relative.length())
		_bodyToMove.rotation_degrees.y -= _mouseSens * _mouseMotion.relative.x
		#print(_bodyToMove.rotation_degrees.y)
		$Camera3D.rotation_degrees.x -= _mouseSens * _mouseMotion.relative.y
